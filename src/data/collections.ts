export const products = [
  {
    id: 1,
    name: "Gentle Pup Soap",
    price: 11.99,
    category: "Natural",
    image: "images/products/dog.jpg",
    description: "A mild, pet-friendly soap bar made for dogs with sensitive skin, using gentle oils and natural ingredients.",
    ingredients: "Olive Oil, Coconut Oil, Essential Oil, Sodium Hydroxide"
  },
  {
    id: 2,
    name: "Mint Refresh Salt Bar",
    price: 11.99,
    category: "Natural",
    image: "images/products/mintsalt.jpg",
    description: "A revitalizing mint-infused bar that exfoliates gently with natural salt while delivering a crisp, cooling sensation.",
    ingredients: "Olive Oil, Coconut Oil, Salt, French Clay, Essential Oil, Sodium Hydroxide"
  },
  {
    id: 3,
    name: "Lavender Calm",
    price: 11.99,
    category: "Floral",
    image: "images/products/lavender.jpg",
    description: "A soothing lavender-infused bar designed to relax the mind and soften the skin with every bath.",
    ingredients: "Olive Oil, Coconut Oil, Corn Flower, Purple Mica Powder, Milk, Essential Oil, Sodium Hydroxide"
  },
  {
    id: 4,
    name: "Green Tea Glow",
    price: 11.99,
    category: "Herbal",
    image: "images/products/tea.jpg",
    description: "A calming green tea bar packed with antioxidants to soothe, nourish, and promote radiant skin.",
    ingredients: "Olive Oil, Coconut Oil, Matcha Powder, Gold Mica, Essential Oil, Sodium Hydroxide"
  },
  {
    id: 5,
    name: "Cherry Blossom Breeze",
    price: 12.99,
    category: "Floral",
    image: "images/products/sakura.jpg",
    description: "A light, airy cherry blossom scent that softens the skin and calms the senses with every wash.",
    ingredients: "Olive Oil, Coconut Oil, Shea Butter, Sunflower Seed Oil, Essential Oil, French Pink Clay, Sodium Hydroxide"
  },
  {
    id: 6,
    name: "Coffee Morning Scrub",
    price: 11.99,
    category: "Specialty",
    image: "images/products/coffee.jpg",
    description: "Awaken your senses with a rich, coffee-infused exfoliating bar that smooths and revitalizes the skin.",
    ingredients: "Olive Oil, Beeswax, Coconut Oil, Sunflower Seed Oil, Shea Fruit Butter, Essential Oil, Cocoa Powder, Coffee Grounds, Sodium Hydroxide"
  },
  {
    id: 7,
    name: "Romantic Rose Garden",
    price: 13.99,
    category: "Floral",
    image: "images/products/lufarose.jpg",
    description: "A luxurious rose-infused bar crafted with floral oils and gentle clays for a soft, elegant cleanse.",
    ingredients: "Olive Oil, Coconut Oil, Castor Oil, Palm Oil, Ginger Water, Essential Oil, Pink Clay, Sodium Hydroxide"
  },
  {
    id: 8,
    name: "Mugwort Herbal Shampoo Bar",
    price: 12.99,
    category: "Herbal",
    image: "images/products/herb.jpg",
    description: "A refreshing mugwort shampoo bar that clarifies the scalp, reduces buildup, and leaves hair feeling light and clean.",
    ingredients: "Sodium Cocoyl Isethionate, Sodium Lauryl Sulfoacetate, Mugwort Powder, Essential Oil, Castor Oil"
  },
  {
    id: 9,
    name: "Ocean Mint Salt Bar",
    price: 11.99,
    category: "Natural",
    image: "images/products/oceansalt.jpg",
    description: "Inspired by the ocean breeze, this cooling mint bar blends sea salt with refreshing botanicals for a smooth, invigorating cleanse.",
    ingredients: "Olive Oil, Coconut Oil, Salt, Milk, Essential Oil, Sodium Hydroxide"
  },
  {
    id: 10,
    name: "Citrus Sunrise",
    price: 11.99,
    category: "Citrus",
    image: "images/products/orange.jpg",
    description: "A bright and refreshing orange soap that energizes the skin with a sweet citrus aroma.",
    ingredients: "Olive Oil, Coconut Oil, Salt, Turmic Powder, Orange Zest, Essential Oil, Sodium Hydroxide"
  },
  {
    id: 11,
    name: "Purifying Charcoal Detox",
    price: 14.99,
    category: "Specialty",
    image: "images/products/charcoal.jpg",
    description: "A powerful deep-cleansing bar enriched with activated charcoal to draw out impurities and refresh the skin.",
    ingredients: "Olive Oil, Coconut Oil, Shea Butter, Essential Oils, Bamboo Charcoal Powder, Sodium Hydroxide"
  }
];
